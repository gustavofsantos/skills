...
| `PostToolUse` | `hooks/parse_session.py` | Reads new JSONL entries since last cursor, runs MapReduce to pair tool events, appends structured log to `$PERSONAL_SESSIONS_DIR` (defaults to `~/.claude/custom-sessions/`). |
